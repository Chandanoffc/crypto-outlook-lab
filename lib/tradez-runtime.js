...
const { hasDatabase, getRuntimeState, listTradeEventsByStrategy, queryCompetitionStats } = require("./neon-db");
const { isDiscordWebhook, resolveBaseUrl, sendDiscordNotify } = require("./notify-client");
const {
  QUOTE_ASSET,
  sanitizeNumber, nullableNumber, nullableTimestamp,
  average, pctChange, pctChangeFromLookback, latestDefinedValue,
  priceDigits, formatPrice, formatPercent, formatBannerPair,
  mapKlineEntry, analyzeTakerLongShort,
  sanitizeActivityEntry, updateAlertDeliveryResult, setRuntimeStatus,
  scoreSymbolCandidate, perpUniverseSymbols,
  fetchJson, safeInsertSignalEvent, safeInsertTradeEvent,
} = require("./engine-core");
...
const TRADEZ_AUTO_MAX_CONCURRENT_TRADES = 8;
const TRADEZ_AUTO_MAX_NEW_TRADES = 3;
...
const MIN_EMA_SEPARATION_ATR = 0.02;
const MAX_STALE_SIGNAL_BARS = 16;
const MAX_AUTO_ENTRY_SIGNAL_BARS = 12;
const MAX_POST_TOUCH_EXTENSION_ATR = 4.2;
const MIN_VISIBLE_SIGNAL_RR = 1.0;
const MIN_EXECUTION_RR = 1.0;
const MIN_VISIBLE_SIGNAL_VOLUME_FACTOR = 0.6;
const MIN_AUTO_EXECUTION_VOLUME_FACTOR = 0.6;
const STRICT_LEVEL_TOUCH_BUFFER_ATR = 0.05;
const STRICT_LEVEL_RECLAIM_BUFFER_ATR = 0.04;
const MAX_EXECUTION_DISTANCE_FROM_TOUCH_ATR = 3.2;
const LIVE_ENTRY_BUFFER_ATR = 1.8;
const TRADEZ_AUTO_EXECUTION_THRESHOLD_BUFFER = 0;
const TRADEZ_AUTO_MIN_EXECUTION_THRESHOLD = 72;
const TRADEZ_FUNDING_CROWD_REJECT_PCT = 0.04;
const TRADEZ_DRAWDOWN_HALF_FLOOR = 0.80;   // below 80% of start → half sizing
const TRADEZ_DRAWDOWN_BLOCK_FLOOR = 0.65;  // below 65% of start → block new trades
...
function defaultRuntimeState() {
  return {
    ...
    alertDelivery: {
      ...
      notifyExits: true,
      lastResultAt: 0,
      lastResultMessage: "Background Discord delivery not configured.",
...
function isClosingTradeEvent(eventType) {
  const normalized = String(eventType || "").toLowerCase();
  return (
    normalized === "tp_hit" ||
    normalized === "sl_hit" ||
    normalized === "break_even_exit"
  );
}

function sanitizeLastFunnel(raw) {
  if (!raw || typeof raw !== "object") return null;
  const n = (v) => sanitizeNumber(v, 0);
  return {
    withSignal: n(raw.withSignal),
    passedQuality: n(raw.passedQuality),
    passedRR: n(raw.passedRR),
    passedFresh: n(raw.passedFresh),
    passedFlow: n(raw.passedFlow),
    passedFunding: n(raw.passedFunding),
    passedRetest: n(raw.passedRetest),
    passedVolume: n(raw.passedVolume),
    passedExtension: n(raw.passedExtension),
    passedExecutable: n(raw.passedExecutable),
    threshold: n(raw.threshold),
    recordedAt: n(raw.recordedAt),
  };
}
  ...
  const sanitized = {
    ...
    lastStatusTone:
      rawState.lastStatusTone === "up" || rawState.lastStatusTone === "down"
        ...
        : "neutral",
    alertDelivery: {
      discordWebhook: String(rawState.alertDelivery?.discordWebhook || "").trim(),
      notifyEntries:
        typeof rawState.alertDelivery?.notifyEntries === "boolean"
          ? rawState.alertDelivery.notifyEntries
          : base.alertDelivery.notifyEntries,
      notifyExits:
        typeof rawState.alertDelivery?.notifyExits === "boolean"
          ? rawState.alertDelivery.notifyExits
          : base.alertDelivery.notifyExits,
      lastResultAt: sanitizeNumber(rawState.alertDelivery?.lastResultAt, 0),
      lastResultMessage: String(
        rawState.alertDelivery?.lastResultMessage || base.alertDelivery.lastResultMessage
      ).slice(0, 240),
      lastResultTone:
        rawState.alertDelivery?.lastResultTone === "up" || rawState.alertDelivery?.lastResultTone === "down"
          ? rawState.alertDelivery.lastResultTone
          : "neutral",
    },
    lastFunnel: sanitizeLastFunnel(rawState.lastFunnel),
    backgroundManaged: true,
    freshStart: Boolean(rawState.freshStart),
    resetAt: nullableTimestamp(rawState.resetAt),
  };
  normalizeTradezResearchBook(sanitized);
  return sanitized;
}

function buildResetRuntimeState(currentState = {}) {
  const sanitizedCurrent = sanitizeRuntimeState(currentState);
  return sanitizeRuntimeState({
    ...defaultRuntimeState(),
    alertDelivery: sanitizedCurrent.alertDelivery,
    freshStart: true,
    resetAt: Date.now(),
    lastStatusMessage: "Background Auto Trade 2 engine reset. Starting from a fresh book.",
    lastStatusTone: "neutral",
  });
}
...
async function recoverRuntimeStateFromTradeEvents(stateInput = {}) {
  ...
  const result = await listTradeEventsByStrategy("ema_book", 1000);
  ...
  for (const row of rows) {
    ...
    const current = tradeMap.get(row.trade_id) || {
      ...
      touch: String(metadata.touch || ""),
      strength: String(metadata.strength || ""),
      entryPrice: nullableNumber(row.entry_price) ?? 0,
      entryZoneLow: nullableNumber(metadata.entryZoneLow) ?? nullableNumber(row.entry_price) ?? 0,
      entryZoneHigh: nullableNumber(metadata.entryZoneHigh) ?? nullableNumber(row.entry_price) ?? 0,
      stopLoss: nullableNumber(row.stop_loss) ?? 0,
      tp1: nullableNumber(row.tp1) ?? 0,
      tp2: nullableNumber(row.tp2) ?? 0,
      currentTarget: nullableNumber(row.tp2 ?? row.tp1) ?? 0,
      tp1Hit: false,
      quantity: nullableNumber(row.quantity) ?? 0,
      leverage: nullableNumber(row.leverage) ?? TRADEZ_AUTO_LEVERAGE,
      ...
      executionMode: String(metadata.executionMode || "demo"),
    };

    current.symbol = row.symbol || current.symbol;
    current.side = row.side === "Short" ? "Short" : "Long";
    current.touch = String(metadata.touch || current.touch || "");
    current.strength = String(metadata.strength || current.strength || "");
    current.entryPrice = nullableNumber(row.entry_price) ?? current.entryPrice;
    current.entryZoneLow = nullableNumber(metadata.entryZoneLow) ?? current.entryZoneLow;
    current.entryZoneHigh = nullableNumber(metadata.entryZoneHigh) ?? current.entryZoneHigh;
    current.stopLoss = nullableNumber(row.stop_loss) ?? current.stopLoss;
    current.tp1 = nullableNumber(row.tp1) ?? current.tp1;
    current.tp2 = nullableNumber(row.tp2) ?? current.tp2;
    current.currentTarget = current.tp2 || current.tp1 || current.currentTarget;
    ...
    current.pricePrecision = nullableNumber(metadata.pricePrecision) ?? current.pricePrecision;
    ...
    if (isClosingTradeEvent(eventType)) {
    ...
    if (eventType === "tp1_hit") {
      current.tp1Hit = true;
      current.stopLoss = current.entryPrice;
      current.currentTarget = current.tp2 || current.currentTarget;
    }
  ...
  const openTrades = [];
  const closedTrades = [];
  let latestBalance = TRADEZ_AUTO_START_BALANCE;
  ...
  for (const trade of tradeMap.values()) {
    ...
    const isClosed = isClosingTradeEvent(lastEventType);
    latestScanAt = Math.max(latestScanAt, Number(trade.closedAt || trade.openedAt || 0));
    if (isClosed && nullableNumber(trade.balanceAfter) !== null) {
    ...
    if (isClosed) {
      closedTrades.push(
        sanitizeTrade({
          ...trade,
          reason:
            lastEventType === "tp_hit" ? "TP" : lastEventType === "break_even_exit" ? "BE" : "SL",
        })
      );
    } else {
  ...
  return {
    ...
    state: sanitizeRuntimeState({
      ...
      openTrades,
      closedTrades: closedTrades.sort((a, b) => (b.closedAt || 0) - (a.closedAt || 0)).slice(0, 100),
      lastScanAt: latestScanAt,
      lastStatusMessage: openTrades.length
        ? `Recovered ${openTrades.length} Auto Trade 2 position${openTrades.length > 1 ? "s" : ""} from persisted event history.`
...
async function fetchUniverseTickers() {
  ...
  try {
    ...
    try {
      const spotTickers = await fetchJson(`${BINANCE_SPOT_BASE_URL}/api/v3/ticker/24hr`, "Spot 24H tickers");
...
async function fetchSnapshotViaMarketApi(token, baseUrl) {
  return fetchJson(
    `${resolveBaseUrl(baseUrl)}/api/market?symbol=${encodeURIComponent(token)}&interval=${encodeURIComponent(STRATEGY_INTERVAL)}`,
    "Runtime market snapshot"
  );
}

async function fetchDirectSnapshot(token, baseUrl) {
  try {
    const payload = await fetchSnapshotViaMarketApi(token, baseUrl);
    const candles = Array.isArray(payload.candles) ? payload.candles.map(mapKlineEntry) : [];
    if (!candles.length || !payload.ticker) {
      throw new Error(`Core perpetual market data is unavailable for ${token}.`);
    }
    return {
      token: payload.token,
      ...
      depth: payload.depth || { bids: [], asks: [] },
      trades: payload.trades || [],
      premiumIndex: payload.premiumIndex || null,
      openInterestHistory: Array.isArray(payload.openInterestHistory) ? payload.openInterestHistory : [],
      globalLongShort: Array.isArray(payload.globalLongShort) ? payload.globalLongShort : [],
      takerLongShort: Array.isArray(payload.takerLongShort) ? payload.takerLongShort : [],
    };
  } catch (_marketApiError) {
    // Fall back to the direct exchange path below.
  }
...
function buildTradezSignals(snapshot, quoteVolume = 0) {
  ...
  for (let index = completedLimit; index < candles.length - 1; index += 1) {
    ...
    if (!hasGoodTradingVolume(quoteVolume)) qualityScore -= 10;
    if (rejectionVolumeFactor < MIN_VISIBLE_SIGNAL_VOLUME_FACTOR) qualityScore -= 6;
    if (!flowAligned) qualityScore -= 10;
    if (!touch20 && !touch50 && !useLevelTwo) qualityScore -= 6;
    ...
    if (side === "Long" && tradeSummary.cvdSlope <= 0) qualityScore -= 6;
    if (side === "Short" && tradeSummary.cvdSlope >= 0) qualityScore -= 6;

    let qualityCap = 180;
    if (!hasGoodTradingVolume(quoteVolume)) qualityCap = Math.min(qualityCap, 122);
    if (rejectionVolumeFactor < MIN_VISIBLE_SIGNAL_VOLUME_FACTOR) qualityCap = Math.min(qualityCap, 130);
    if (!flowAligned) qualityCap = Math.min(qualityCap, 128);
    if (!touch20 && !touch50 && !useLevelTwo) qualityCap = Math.min(qualityCap, 126);
    const elite =
      flowAligned &&
...
function highQualityTradezAutoCandidates(candidates, threshold) {
  ...
  // Step-by-step funnel — each stage records its own exit count.
  const funnel = {
    withSignal: 0,
    passedQuality: 0,
    passedRR: 0,
  ...
  const built = candidates.map(buildTradezAutoCandidate).filter(Boolean);
  funnel.withSignal = built.length;

  const q = built.filter((c) => c.qualityScore >= executionThreshold);
  funnel.passedQuality = q.length;

  const rr = q.filter((c) => c.paperTrade.rr >= MIN_EXECUTION_RR);
  funnel.passedRR = rr.length;

  const fresh = rr.filter((c) => (c.activeSignal?.sinceTouchBars || 0) <= MAX_AUTO_ENTRY_SIGNAL_BARS);
  funnel.passedFresh = fresh.length;

  const flow = fresh.filter((c) => {
    const fc = c.activeSignal?.flowConfirmations || 0;
    const htf = Boolean(c.activeSignal?.higherTimeframeConfirmed);
    return fc >= 2 || (fc >= 1 && htf);
  });
  funnel.passedFlow = flow.length;

  const funding = flow.filter((c) => {
    const f = c.fundingRate || 0;
    const side = c.activeSignal?.side || "";
    return !(side === "Long" && f > TRADEZ_FUNDING_CROWD_REJECT_PCT) &&
           !(side === "Short" && f < -TRADEZ_FUNDING_CROWD_REJECT_PCT);
  });
  funnel.passedFunding = funding.length;

  const retest = funding.filter((c) => (c.activeSignal?.retestCount || 0) <= 4);
  funnel.passedRetest = retest.length;

  const vol = retest.filter((c) => (c.activeSignal?.volumeFactor || 0) >= MIN_AUTO_EXECUTION_VOLUME_FACTOR);
  funnel.passedVolume = vol.length;

  const ext = vol.filter(
    (c) =>
      !Number.isFinite(c.activeSignal?.extensionFromTouch) ||
      c.activeSignal.extensionFromTouch <= c.latestAtr * MAX_POST_TOUCH_EXTENSION_ATR
  );
  funnel.passedExtension = ext.length;

  const executable = ext.filter(candidateIsExecutable);
  funnel.passedExecutable = executable.length;

  const eligible = executable.sort((a, b) => {
    const bSeen = b.identifiedAt || b.activeSignal?.detectedAt || 0;
    const aSeen = a.identifiedAt || a.activeSignal?.detectedAt || 0;
    return b.qualityScore - a.qualityScore || bSeen - aSeen;
  });

  return { eligible, funnel };
}

function buildTradezEntryAlertEvent(trade) {
  const pair = formatBannerPair(trade.symbol);
  const title = `EMA Book ${trade.side} ${trade.symbol}`;
  const event = {
    time: trade.openedAt,
    title,
    symbol: trade.symbol,
  ...
  event.message = [
    `NEW SIGNAL • ${pair} • ${event.direction}`,
    "",
    `Strategy: ${event.strategy}`,
    `Touch: ${trade.touch || "EMA pullback"}`,
    `Quality: Q${Math.round(Number(trade.qualityScore) || 0)}`,
    "",
    `Entry: ${formatPrice(trade.entryPrice, trade.pricePrecision || 2)}`,
    `Stop Loss: ${formatPrice(trade.stopLoss, trade.pricePrecision || 2)}`,
    `TP1: ${formatPrice(trade.tp1, trade.pricePrecision || 2)}`,
    `TP2: ${formatPrice(trade.tp2, trade.pricePrecision || 2)}`,
    `RR: ${Number.isFinite(Number(trade.rr)) ? Number(trade.rr).toFixed(2) + "R" : "—"}`,
  ].join("\n");
  event.formattedMessage = event.message;
  return event;
}

function buildTradezProgressAlertEvent(kind, trade, closedTrade = trade) {
  const pair = formatBannerPair(trade.symbol);
  const titleMap = {
  ...
  const event = {
    time: closedTrade.closedAt || Date.now(),
    title: titleMap[kind] || `EMA Book Update ${trade.symbol}`,
    symbol: trade.symbol,
    pair,
    type: "tradez",
    direction: trade.side,
  ...
  const exitLabel = kind === "tp1" ? trade.tp1 : closedTrade.exitPrice || trade.stopLoss || trade.entryPrice;
  event.message = [
    kind === "tp1"
      ? "SUCCESS.. TP1 Hit✅"
      : kind === "tp"
    ...
    `${pair} • ${trade.side} • ${trade.leverage || TRADEZ_AUTO_LEVERAGE}x`,
    `Entry ${formatPrice(trade.entryPrice, trade.pricePrecision || 2)} • Exit ${formatPrice(
      exitLabel,
      closedTrade.pricePrecision || trade.pricePrecision || 2
    )}`,
    Number.isFinite(Number(closedTrade.returnPct))
      ? `Return ${formatPercent(closedTrade.returnPct)} • ${formatCompactUsd(closedTrade.pnlUsd, 2)}`
      : kind === "tp1"
        ? `TP1 ${formatPrice(trade.tp1, trade.pricePrecision || 2)} armed, stop moved to entry`
        : "",
  ]
    .filter(Boolean)
    .join("\n");
...
async function maybeSendTradezRuntimeAlert(state, event, meta, baseUrl) {
  const delivery = state.alertDelivery || {};
  if (!isDiscordWebhook(delivery.discordWebhook)) return;
  try {
    await sendDiscordNotify(baseUrl, delivery.discordWebhook, event.bannerTitle || event.title, event, meta);
    updateAlertDeliveryResult(state, "up", "Background Auto Trade 2 Discord delivery succeeded.");
  } catch (error) {
    updateAlertDeliveryResult(state, "down", error.message || "Background Auto Trade 2 Discord delivery failed.");
    logActivity(state, `Tradez Discord delivery failed • ${error.message || "Unknown error"}`, "down");
  }
}

// Returns a 0.5–1.25 multiplier based on signal quality score.
// Higher-conviction signals get proportionally more capital.
function qualityMarginMultiplier(score) {
...
// Returns a drawdown multiplier: 1.0 = normal, 0.5 = half sizing, 0.0 = block.
function drawdownMultiplier(balance, startingBalance) {
  if (!startingBalance || startingBalance <= 0) return 1.0;
  const ratio = balance / startingBalance;
  if (ratio < TRADEZ_DRAWDOWN_BLOCK_FLOOR) return 0.0;
  if (ratio < TRADEZ_DRAWDOWN_HALF_FLOOR) return 0.5;
  return 1.0;
}

async function openTradezPaperTrade(state, candidate, baseUrl, houseOpenSymbols = new Set()) {
  ...
  // Cross-engine symbol lock — block opposite-direction trades on same symbol as house-runtime.
  if (houseOpenSymbols.has(candidate.symbol)) return false;

  // Drawdown circuit breaker.
  const ddMult = drawdownMultiplier(state.balance, state.startingBalance);
  if (ddMult === 0) return false;
  ...
  const qualityMult = qualityMarginMultiplier(candidate.qualityScore);
  const marginBudget = Math.min(
    freeCapital,
    Math.max(state.startingBalance * 0.03, freeCapital / slotsRemaining) * qualityMult * ddMult
  );
  ...
  if (state.alertDelivery?.notifyEntries !== false) {
    await maybeSendTradezRuntimeAlert(
      state,
      buildTradezEntryAlertEvent(trade),
      {
        source: "tradez_cron",
        strategy: "ema_book",
...
async function closeTradezPaperTrade(state, tradeId, reason, exitPrice, precisionHint, baseUrl) {
  ...
  if (state.alertDelivery?.notifyExits !== false) {
    await maybeSendTradezRuntimeAlert(
      state,
      buildTradezProgressAlertEvent(reason === "TP" ? "tp" : reason === "SL" ? "sl" : "be", trade, closedTrade),
      {
        source: "tradez_cron",
        strategy: "ema_book",
        eventType: reason === "TP" ? "tp_hit" : reason === "SL" ? "sl_hit" : "break_even_exit",
      },
      baseUrl
    );
  }
...
async function pruneUnavailableOpenTrades(state, universe, baseUrl) {
  if (!state.openTrades.length) return;
  const activeSymbols = new Set((universe || []).map((item) => item.symbol).filter(Boolean));
  for (const trade of [...state.openTrades]) {
    if (activeSymbols.has(trade.symbol)) continue;
    const exitPrice = Number.isFinite(trade.lastPrice) ? trade.lastPrice : trade.entryPrice;
    await closeTradezPaperTrade(state, trade.id, "BE", exitPrice, trade.pricePrecision, baseUrl);
  }
}

async function refreshTradezPaperTrades(state, candidates, tickerLookup = universeTickerMap, baseUrl) {
  if (!state.openTrades.length) return;
  ...
  for (const trade of [...state.openTrades]) {
    ...
    const ticker = tickerLookup.get(trade.symbol);
    const currentPrice = Number.isFinite(candidate?.currentPrice)
      ? candidate.currentPrice
      : Number(ticker?.lastPrice);
    if (!Number.isFinite(currentPrice)) continue;
    trade.lastPrice = currentPrice;
    if (Number.isFinite(candidate?.pricePrecision)) trade.pricePrecision = candidate.pricePrecision;
    ...
    if (hitTp1) {
      ...
      logActivity(
        ...
        `${trade.symbol} hit TP1. Stop moved to entry while the runner targets ${formatPrice(
          trade.tp2,
          candidate?.pricePrecision ?? trade.pricePrecision
        )}.`,
      ...
      if (state.alertDelivery?.notifyExits !== false) {
        await maybeSendTradezRuntimeAlert(
          state,
          buildTradezProgressAlertEvent("tp1", trade),
          {
            source: "tradez_cron",
            strategy: "ema_book",
            eventType: "tp1_hit",
...
async function openTradezQualifiedTrades(state, candidates, baseUrl, houseOpenSymbols = new Set()) {
  const { eligible: allEligible, funnel } = highQualityTradezAutoCandidates(candidates, state.qualityThreshold);
  ...
  for (const candidate of eligible) {
    ...
    if (await openTradezPaperTrade(state, candidate, baseUrl, houseOpenSymbols)) opened.push(candidate);
  }
  return opened;
}

function buildCompetitionStatsForStrategy(rows, strategy) {
  const stratRows = (rows || []).filter((row) => row.strategy === strategy);
  const tpRow = stratRows.find((row) => row.event_type === "tp_hit") || {};
  const slRow = stratRows.find((row) => row.event_type === "sl_hit") || {};
  const beRow = stratRows.find((row) => row.event_type === "break_even_exit") || {};
  ...
  return { tpCount, slCount, beCount, resolved, winRate, avgGainPct, avgLossPct, totalPnlUsd };
}

async function maybeSendDailyCompetitionReport(state, baseUrl) {
  if (!isDiscordWebhook(state.alertDelivery?.discordWebhook)) return;
  const now = Date.now();
  const previousDayKey = utcDayKey(now - UTC_DAY_MS);
  if (state.lastDailyBriefingUtcDate === previousDayKey) return;

  state.lastDailyBriefingUtcDate = previousDayKey;

  try {
    const competitionResult = await queryCompetitionStats(24);
    const rows = competitionResult?.rows || [];
    const house = buildCompetitionStatsForStrategy(rows, "house_trend");
    const ema = buildCompetitionStatsForStrategy(rows, "ema_book");

    const houseScore = (house.winRate > ema.winRate ? 1 : 0) + (house.totalPnlUsd > ema.totalPnlUsd ? 2 : 0);
    const emaScore = (ema.winRate > house.winRate ? 1 : 0) + (ema.totalPnlUsd > house.totalPnlUsd ? 2 : 0);
    const leaderLabel =
      houseScore > emaScore
        ? "🏆 Auto Trade leads today"
        : emaScore > houseScore
          ? "🏆 Auto Trade 2 leads today"
          : "🤝 Tied session";

    const formatPct = (value) => `${value >= 0 ? "+" : ""}${value.toFixed(2)}%`;
    const formatUsd = (value) => `${value >= 0 ? "+" : "-"}$${Math.abs(value).toFixed(2)}`;

    const lines = [
      "⚔️  SOLORIS — DAILY COMPETITION REPORT",
      `📅 ${previousDayKey} (UTC) · 24H window`,
      "",
      `🔵 AUTO TRADE (House Trend · 15m)`,
      `   Trades: ${house.resolved}  |  TPs: ${house.tpCount}  |  SLs: ${house.slCount}  |  BEs: ${house.beCount}`,
      house.resolved > 0
        ? `   Win Rate: ${house.winRate.toFixed(1)}%  |  Avg Gain: ${formatPct(house.avgGainPct)}  |  Avg Loss: ${formatPct(house.avgLossPct)}`
        : "   No resolved trades this period.",
      `   Realized P&L: ${formatUsd(house.totalPnlUsd)}`,
      "",
      `🟣 AUTO TRADE 2 (EMA Signals · 1H)`,
      `   Trades: ${ema.resolved}  |  TPs: ${ema.tpCount}  |  SLs: ${ema.slCount}  |  BEs: ${ema.beCount}`,
      ema.resolved > 0
        ? `   Win Rate: ${ema.winRate.toFixed(1)}%  |  Avg Gain: ${formatPct(ema.avgGainPct)}  |  Avg Loss: ${formatPct(ema.avgLossPct)}`
        : "   No resolved trades this period.",
      `   Realized P&L: ${formatUsd(ema.totalPnlUsd)}`,
      "",
      leaderLabel,
    ];

    const message = lines.join("\n");
    await sendDiscordNotify(baseUrl, state.alertDelivery.discordWebhook, "Daily Competition Report", {
      title: "Daily Competition Report",
      message,
      formattedMessage: message,
      deliveryType: "competition_report",
      time: now,
    }, { source: "tradez_cron", strategy: "competition", eventType: "daily_report" });

    logActivity(state, `Daily competition report sent for ${previousDayKey}.`, "up");
  } catch (error) {
    logActivity(state, `Daily competition report failed: ${error.message || "unknown error"}`, "down");
  }
}
...
async function runTradezScan(stateInput = {}, options = {}) {
  ...
  if (
    isDiscordWebhook(state.alertDelivery?.discordWebhook) &&
    (!state.alertDelivery?.lastResultAt ||
      /not configured/i.test(String(state.alertDelivery?.lastResultMessage || "")) ||
      /failed \(401\)/i.test(String(state.alertDelivery?.lastResultMessage || "")))
  ) {
    updateAlertDeliveryResult(state, "neutral", "Background Auto Trade 2 Discord delivery is armed.");
  }

  try {
    const universe = await getPerpUniverse();
    await pruneUnavailableOpenTrades(state, universe, baseUrl);
    ...
    const analyses = await mapWithConcurrency(batch, ANALYSIS_CONCURRENCY, async (symbolInfo) => {
      try {
        const snapshot = await fetchDirectSnapshot(symbolInfo.symbol, baseUrl);
    ...
    await refreshTradezPaperTrades(state, analyzedCandidates, universeTickerMap, baseUrl);

    // Cross-engine lock: read house-runtime open positions once per scan (non-blocking).
    let houseOpenSymbols = new Set();
    if (hasDatabase()) {
      try {
        const houseRaw = await getRuntimeState("house_auto_trade");
        houseOpenSymbols = new Set((houseRaw?.state?.openTrades || []).map((t) => t.symbol));
      } catch (_) {
        // Non-blocking — if DB read fails, proceed without the lock.
      }
    }

    const openedTrades = state.autoEnabled
      ? await openTradezQualifiedTrades(state, state.lastCandidates, baseUrl, houseOpenSymbols)
      : [];

    const best = state.lastCandidates[0];
    const degradedSummary = snapshotFailures
      ? ` Scanner degraded: ${snapshotFailures}/${batch.length} symbol snapshots failed.`
      : "";
    if (openedTrades.length) {
      const lead = openedTrades[0];
      setRuntimeStatus(
        state,
        `Opened ${openedTrades.length} new EMA Signals trade${openedTrades.length > 1 ? "s" : ""}. ${state.openTrades.length}/${TRADEZ_AUTO_MAX_CONCURRENT_TRADES} Auto Trade 2 positions are active, led by ${lead.symbol}.${degradedSummary}`,
        lead.activeSignal?.tone || "up"
      );
    } else if (best) {
      setRuntimeStatus(
        state,
        `${state.lastCandidates.filter((candidate) => candidate.qualityScore >= state.qualityThreshold).length} Tradez setups qualified. ${best.symbol} leads with Q${best.qualityScore}.${degradedSummary}`,
        snapshotFailures ? "neutral" : "up"
      );
    } else {
      setRuntimeStatus(
        state,
        snapshotFailures
          ? `Tradez scan complete, but scanner is degraded. ${snapshotFailures}/${batch.length} symbol snapshots failed and no setup currently clears the active quality filter.`
          : "Tradez scan complete. No setup currently clears the active quality filter.",
        snapshotFailures ? "down" : "neutral"
      );
    }
    ...
    if (!manual) {
      await maybeSendDailyCompetitionReport(state, baseUrl);
    }
