...
async function fetchSymbolData(symbol) {
  const [klinesResult, klines4hResult, klines1dResult, tickerResult, premiumResult] = await Promise.allSettled([
...
function detectSignal(data, pricePrecision = 2, context = {}) {
  ...
  // ─── 1D macro alignment filter ──────────────────────────────────────────────
  // Require 1D EMA20/50 to agree with the trade direction. Daily downtrend
  // means the higher-timeframe bias is bearish — taking longs there adds structural
  // headwind on top of the 4H filter and is the primary source of streak losses.
  const candles1d = data.candles1d || [];
  if (candles1d.length >= 55) {
    ...
    if (ema20_1d[last1d] != null && ema50_1d[last1d] != null) {
      const trend1d = ema20_1d[last1d] > ema50_1d[last1d] ? "up" : "down";
      if (side === "Long"  && trend1d === "down") return null;
      if (side === "Short" && trend1d === "up")   return null;
    }
  }
  ...
  // ADX < 18 = directionless market → S/R and EMA touches are noise, not setups.
