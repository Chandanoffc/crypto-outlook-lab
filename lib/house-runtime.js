...
async function openTradeFromCandidate(state, candidate, baseUrl) {
...
async function openQualifiedTrades(state, candidates, baseUrl) {
  const opened = [];
  ...
  for (const candidate of eligible) {
    ...
    if (await openTradeFromCandidate(state, candidate, baseUrl)) {
      opened.push({
        strategyId: spec.id,
        strategyLabel: spec.label,
        symbol: candidate.symbol,
...
async function runHouseScan(stateInput = {}, options = {}) {
  ...
  try {
    ...
    const opened = await openQualifiedTrades(state, candidates, baseUrl);
