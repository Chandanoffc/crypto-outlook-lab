...
function simulateTrade(signal, candles1h, entryIndex, runtime) {
  ...
  function finish(outcome, exitPrice, totalPnlPct, exitTime) {
    return {
      ...
      pnlPct: totalPnlPct,
      tp1Hit,
      barsHeld: Math.max(1, Math.round((exitTime - entryTime) / (60 * 60 * 1000))),
    };
  }
}
...
async function replaySymbol(symbol, candles1h, candles4h, ticker, runtime, options) {
  const trades = [];
  ...
  for (let i = WARMUP_1H; i < candles1h.length; i += stepHours) {
    ...
    try {
      signal = runtime.detectSignal(
        { symbol, candles1h: slice1h, candles4h: slice4h, ticker, premium: { lastFundingRate: 0 } },
        2
      );
    } catch (_) {
      continue;
...
function summarize(trades, startingBalance) {
  if (!trades.length) {
    return {
      ...
      maxDrawdownPct: 0, finalBalance: startingBalance, returnPct: 0,
  ...
  // Compounding equity curve, in chronological order, for drawdown + final balance.
  const chrono = [...trades].sort((a, b) => a.entryTime - b.entryTime);
  let balance = startingBalance;
  let peak = startingBalance;
  let maxDrawdownPct = 0;
  for (const t of chrono) {
    balance *= (1 + t.accountPnlPct / 100);
  ...
  return {
    ...
    returnPct: Number((((balance - startingBalance) / startingBalance) * 100).toFixed(2)),
...
async function runBacktest(opts = {}) {
  ...
  return {
    ...
    notes: [
      ...
      "Per-trade accountPnlPct assumes a flat starting balance (no compounding) for apples-to-apples comparison; the equity-curve summary (finalBalance/returnPct/maxDrawdownPct) applies compounding chronologically.",
    ],
  };
}
