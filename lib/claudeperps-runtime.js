...
async function fetchSymbolData(symbol, baseUrl) {
  const [klines1hResult, klines4hResult, klines1dResult, tickerResult, premiumResult] = await Promise.allSettled([
    ...
    fetchJson(`${BINANCE_FAPI}/klines?symbol=${symbol}&interval=1d&limit=60`, "1D klines"),
  ...
  return {
    ...
    candles1d: klines1dResult.status === "fulfilled" ? klines1dResult.value.map(mapKlineEntry) : [],
...
function detectSignal(data, pricePrecision = 2, context = {}) {
  const { symbol, candles1h, candles4h, candles1d, ticker, premium } = data;
  ...
  const closes1d = (candles1d || []).length >= 55 ? candles1d.map(c => c.close) : null;
  ...
  // 1D trend — requires EMA20 > EMA50 on daily timeframe to confirm macro structure
  let trend1d = null;
  if (closes1d) {
    const ema20_1d = ema(closes1d, 20);
    const ema50_1d = ema(closes1d, 50);
    const last1d = closes1d.length - 1;
    if (ema20_1d[last1d] != null && ema50_1d[last1d] != null) {
      trend1d = ema20_1d[last1d] > ema50_1d[last1d] ? "up" : "down";
    }
  }
  ...
  // 0d. 1D macro alignment — both 4H and 1D EMA20/50 must agree with the trade direction.
  // Longs into a daily downtrend (EMA20 < EMA50 on 1D) are the primary source of streak losses
  // in ranging/bear markets. Hard block when 1D trend opposes the signal.
  if (trend1d !== null) {
    if (isLong  && trend1d === "down") return null;
    if (!isLong && trend1d === "up")   return null;
  }
