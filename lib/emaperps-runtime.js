...
const MIN_ALERT_QUALITY = 83;                         // minimum quality to fire a Discord alert
...
function detectSignal(data, pricePrecision = 2) {
  ...
  //
  // HARD GATE for all G/H types: the entry candle MUST show a rejection wick —
  // an EMA touch without visible rejection is a pass-through (breakout), not a
  // reversal. G2 (0% WR, 5 trades) was consistently losing because price touched
  // EMA50 in a downtrend and kept rallying rather than rejecting. Requiring a wick
  // makes the entry condition structurally sound: we only enter when the candle
  // itself shows the EMA acting as resistance/support.
  // G2/H2 base quality raised 78→80; G1/H1 raised 72→75 to reflect that EMA50
  // touches are higher-conviction entries than EMA20 touches.
  //
  // DISABLED 2026-06-13: even with the wick-rejection gate above, G/H types
  // accounted for 17 of 18 closed paper trades and -$30.32 of -$31.27 total
  // loss (97%). Their 1×ATR SL vs 3-5×ATR TP structure gets blown through by
  // normal crypto volatility (losses of -20% to -63% per trade at 5x). The
  // S/R-based A-F types are structurally sound (slLevel from real S/R) and
  // should carry the strategy until G/H gets a proper SL redesign.
  const ghEnabled = false;
  const ghWickOk = ghEnabled && side === null && (
    // For potential shorts (G1/G2): need upper wick — candle rejected at the EMA high
    (downtrend && (upperWick > body * 0.6 && upperWick > atr * 0.12)) ||
    // For potential longs (H1/H2): need lower wick — candle bounced off the EMA low
    (uptrend && (lowerWick > body * 0.6 && lowerWick > atr * 0.12))
  );
  if (side === null && ghWickOk) {
    if (downtrend && ema50Touch) {
      // Price bounces back up to EMA50 in a downtrend — EMA50 acts as resistance
      side = "Short"; baseQuality = 80; signalType = "G2"; signalLabel = "EMA50 Retest Short";
      tp1 = price - atr * 3; tp2 = price - atr * 5; slLevel = null;
    } else if (downtrend && ema20Touch) {
      side = "Short"; baseQuality = 75; signalType = "G1"; signalLabel = "EMA20 Retest Short";
      tp1 = price - atr * 3; tp2 = price - atr * 5; slLevel = null;
    } else if (uptrend && ema50Touch) {
      // Price pulls back to EMA50 in an uptrend — EMA50 acts as support
      side = "Long"; baseQuality = 80; signalType = "H2"; signalLabel = "EMA50 Pullback Long";
      tp1 = price + atr * 3; tp2 = price + atr * 5; slLevel = null;
    } else if (uptrend && ema20Touch) {
      side = "Long"; baseQuality = 75; signalType = "H1"; signalLabel = "EMA20 Pullback Long";
      tp1 = price + atr * 3; tp2 = price + atr * 5; slLevel = null;
    }
  }
  ...
  // RSI gate — tightened from (38–52 / 48–62) to (42–50 / 50–58) to avoid entries
  // where momentum is already overextended or too weak. The wider band was firing on
  // ~60% of all candles; this narrower mid-range forces entries only when RSI has
  // clear directional room remaining before the next extreme reversal zone.
  const rsiOk = side === "Long"
    ? (rsiLast != null && rsiLast >= 42 && rsiLast <= 50)
    : (rsiLast != null && rsiLast >= 50 && rsiLast <= 58);
  ...
  if (slDistance < atr * 1.5) return null; // SL too tight — skip
  const sl = slRaw;
