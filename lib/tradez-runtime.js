...
const { hasDatabase, getRuntimeState, insertSignalEvent, insertTradeEvent, listTradeEventsByStrategy, queryCompetitionStats } = require("./neon-db");
...
function highQualityTradezAutoCandidates(candidates, threshold) {
  ...
  return candidates
    ...
    .filter((candidate) => {
      ...
      return flowConfirmations >= 2 || (flowConfirmations >= 1 && higherTimeframeConfirmed);
    })
    .filter((candidate) => {
      const funding = candidate.fundingRate || 0;
      const side = candidate.activeSignal?.side || "";
      const overcrowdedLong = side === "Long" && funding > TRADEZ_FUNDING_CROWD_REJECT_PCT;
      const overcrowdedShort = side === "Short" && funding < -TRADEZ_FUNDING_CROWD_REJECT_PCT;
      return !overcrowdedLong && !overcrowdedShort;
    })
...
function updateAlertDeliveryResult(state, tone, message) {
  state.alertDelivery = {
    ...(state.alertDelivery || {}),
    lastResultAt: Date.now(),
    lastResultMessage: String(message || "").slice(0, 240),
    lastResultTone: tone === "up" || tone === "down" ? tone : "neutral",
  };
}
