...
async function refreshPerpsPlaygroundSignals(baseUrl) {
  ...
  if (!symbols.length) {
    return { signals: [], universe, source, warning };
  }
...
async function runPlaygroundScan(stateInput = {}, options = {}) {
  ...
  if (modules.includes(PERPS_MODULE)) {
    try {
      ...
      const { signals, universe, source, warning } = await refreshPerpsPlaygroundSignals(baseUrl);
      state.perps.runtime = {
        backgroundAvailable: true,
      ...
      pushModuleLog(state, PERPS_MODULE, "scanLog", {
        ...
        message: signals.length
          ? `Playground Engine qualified ${signals.length} live perps setup${signals.length === 1 ? "" : "s"} across ${Math.min(universe.length, 15)} symbols.`
          : manual
            ? "Manual perps background refresh completed with no fresh setup."
            : "Perps background refresh completed with no fresh setup.",
      });
