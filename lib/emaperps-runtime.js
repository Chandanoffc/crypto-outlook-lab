...
function detectSignal(data, pricePrecision = 2, context = {}) {
  ...
  if (uptrend && srLevelsUsable) {
    // Long signal cases — SL anchored to S/R level, TP uses ATR for consistent R:R across all coins
    if (ema20Touch && s2Touch)      { side = "Long"; baseQuality = 80; signalType = "A2"; signalLabel = "EMA20 + Support 2"; slLevel = s2; }
    else if (ema20Touch && s1Touch) { side = "Long"; baseQuality = 70; signalType = "A1"; signalLabel = "EMA20 + Support 1"; slLevel = s1; }
    else if (ema50Touch && s2Touch) { side = "Long"; baseQuality = 85; signalType = "B2"; signalLabel = "EMA50 + Support 2"; slLevel = s2; }
    else if (ema50Touch && s1Touch) { side = "Long"; baseQuality = 75; signalType = "B1"; signalLabel = "EMA50 + Support 1"; slLevel = s1; }
    else if (s2Touch)               { side = "Long"; baseQuality = 70; signalType = "C2"; signalLabel = "Support 2 Only";    slLevel = s2; }
    else if (s1Touch)               { side = "Long"; baseQuality = 60; signalType = "C1"; signalLabel = "Support 1 Only";    slLevel = s1; }
  }

  if (downtrend && side === null && srLevelsUsable) {
    // Short signal cases — SL anchored to S/R level, TP uses ATR for consistent R:R across all coins
    if (ema20Touch && r2Touch)      { side = "Short"; baseQuality = 80; signalType = "D2"; signalLabel = "EMA20 + Resistance 2"; slLevel = r2; }
    else if (ema20Touch && r1Touch) { side = "Short"; baseQuality = 70; signalType = "D1"; signalLabel = "EMA20 + Resistance 1"; slLevel = r1; }
    else if (ema50Touch && r2Touch) { side = "Short"; baseQuality = 85; signalType = "E2"; signalLabel = "EMA50 + Resistance 2"; slLevel = r2; }
    else if (ema50Touch && r1Touch) { side = "Short"; baseQuality = 75; signalType = "E1"; signalLabel = "EMA50 + Resistance 1"; slLevel = r1; }
    else if (r2Touch)               { side = "Short"; baseQuality = 70; signalType = "F2"; signalLabel = "Resistance 2 Only";    slLevel = r2; }
    else if (r1Touch)               { side = "Short"; baseQuality = 60; signalType = "F1"; signalLabel = "Resistance 1 Only";    slLevel = r1; }
  }
  ...
  // ATR-based TP1/TP2 for all signal types — ensures consistent R:R across coins
  // regardless of how far apart swing S/R levels happen to be.
  // P/Q signals set their own tp1/tp2 above; A-F signals only set slLevel (for SL).
  if (tp1 == null && side !== null) {
    tp1 = side === "Long" ? price + atr * 2.5 : price - atr * 2.5;
  }
  ...
  if (Array.isArray(candles4h) && candles4h.length >= 55) {
    ...
    if (ema20_4h_last != null && ema50_4h_last != null) {
      ...
      // 4H trend opposing signal: hard block all counter-trend signals.
      // Counter-trend range fades at S/R looked appealing in theory but had poor WR
      // in practice — the 4H trend dominates on longer timeframes and the "exhaustion"
      // conditions (RSI + multi-tested level) are not reliable enough to overcome it.
  ...
  // Minimum R:R gate: TP1 must be at least 2× the SL distance away from entry.
  // At 1:1 you need >50% WR just to break even; 2:1 requires only >33%.
  const rrToTp1 = tp1Dist / (slDistance / price);
  if (rrToTp1 < 2.0) return null;
...
function formatAlertMessage(signal) {
  ...
  const rrToTp1 = signal.entryPrice && signal.sl && signal.tp1
    ? ((Math.abs(signal.tp1 - signal.entryPrice) / Math.abs(signal.entryPrice - signal.sl))).toFixed(2)
    : "–";
  const tp1Pct = signal.entryPrice && signal.tp1
    ? ((Math.abs(signal.tp1 - signal.entryPrice) / signal.entryPrice) * 100).toFixed(2)
    : "–";
  const slPct = signal.entryPrice && signal.sl
  ...
  return [
    ...
    `TP1:    ${fp(signal.tp1)}  (+${tp1Pct}%)`,
    `TP2:    ${fp(signal.tp2)}`,
    `SL:     ${fp(signal.sl)}  (-${slPct}%)`,
    `R:R:    1 : ${rrToTp1}`,
    ``,
...
async function runEmaPerps_Scan(stateInput = {}, options = {}) {
  ...
  // Mark open paper positions against live prices — pass null for baseUrl so
  // this path never fires Discord close alerts. Close alerts are sent exclusively
  // from the action=mark API endpoint (which holds the atomic DB lock), preventing
  // duplicate alerts when a scan and a mark request run with overlapping DB state.
  ...
  try {
    ...
    for (const signal of results.filter(Boolean)) {
      ...
      // Before evicting, capture the old signal's alertedAt so we can inherit the cooldown.
      // Without this, a new candle (new signal ID) would evict the alerted signal and
      // immediately re-alert the same setup, firing every hour per token.
