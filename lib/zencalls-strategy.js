...
// Hard cap on new Discord alerts per scan run — prevents mass-blast even if
// many symbols qualify simultaneously (e.g. after a reset or full state wipe).
...
async function scanSymbol(symbol, price, daily, weekly, monthly, dailyK, state, now, webhook, acc, ptState) {
  // Skip if already active (presence-based dedup — signal stays active until TP/SL hit)
  const alreadyActive = (state.signals || []).some(s => s.symbol === symbol && s.status === "active");
  if (alreadyActive) return;

  // Per-scan alert cap — if we've already sent MAX_ALERTS_PER_SCAN this run, skip
  if (acc.signals >= MAX_ALERTS_PER_SCAN) return;
