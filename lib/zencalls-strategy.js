...
async function scanSymbol(symbol, price, daily, weekly, monthly, dailyK, state, now, webhook, acc, ptState) {
  // Skip if an active signal already exists for this symbol.
  // This prevents re-alerting the same setup every 6h just because the cooldown expired.
  // The signal re-fires only after it closes (TP/SL hit via paper trade checker).
