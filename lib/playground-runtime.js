...
const MAX_BACKGROUND_ALERTS_PER_SCAN = 1;
...
async function sendPerpsAlerts(state, signals, baseUrl) {
  ...
  const candidates = (signals || [])
    .filter(
      (candidate) =>
        candidate.qualityScore >= 44 &&
        withinLookback(candidate.timestamp || Date.now()) &&
        !state.perps.sentIds.includes(candidate.id)
    )
    .sort(
      (left, right) =>
    ...
    .slice(0, MAX_BACKGROUND_ALERTS_PER_SCAN);
...
async function sendDlmmAlerts(state, baseUrl) {
  ...
  const newCalls = state.dlmm.recentCalls
    .filter((call) => call.status === "Open" && withinLookback(call.detectedAt) && !state.dlmm.sentIds.includes(call.id))
    .sort(
      (left, right) =>
        (Number(right.detectedAt || right.lastSeenAt || 0) - Number(left.detectedAt || left.lastSeenAt || 0)) ||
        (Number(right.currentQualityScore || 0) - Number(left.currentQualityScore || 0))
    )
    .slice(0, MAX_BACKGROUND_ALERTS_PER_SCAN);
  let sent = 0;
