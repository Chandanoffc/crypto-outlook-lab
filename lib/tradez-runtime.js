...
async function pruneUnavailableOpenTrades(state, universe) {
  ...
  for (const trade of [...state.openTrades]) {
    ...
    await closeTradezPaperTrade(state, trade.id, "BE", exitPrice, trade.pricePrecision);
  }
}

async function refreshTradezPaperTrades(state, candidates, tickerLookup = universeTickerMap) {
  if (!state.openTrades.length) return;
  ...
  for (const trade of [...state.openTrades]) {
    ...
    if (hitTp2) {
      await closeTradezPaperTrade(state, trade.id, "TP", trade.tp2, candidate?.pricePrecision ?? trade.pricePrecision);
    } else if (hitStop) {
...
async function runTradezScan(stateInput = {}, options = {}) {
  ...
  try {
    const universe = await getPerpUniverse();
    await pruneUnavailableOpenTrades(state, universe);
    ...
    await refreshTradezPaperTrades(state, analyzedCandidates, universeTickerMap);
