...
async function pruneUnavailableOpenTrades(state, universe) {
  ...
  for (const trade of [...state.openTrades]) {
    ...
    await closeTrade(state, trade.id, "BE", exitPrice, trade.pricePrecision);
  }
}
...
async function refreshOpenTrades(state, candidates, tickerLookup = universeTickerMap) {
  if (!state.openTrades.length) return;
  ...
  for (const trade of trades) {
    ...
    if (hitTarget) {
      await closeTrade(state, trade.id, "TP", trade.takeProfit, candidate?.pricePrecision ?? trade.pricePrecision);
    } else if (hitStop) {
      await closeTrade(state, trade.id, "SL", trade.stopLoss, candidate?.pricePrecision ?? trade.pricePrecision);
    }
  }
}
...
async function runHouseScan(stateInput = {}, options = {}) {
  ...
  try {
    ...
    await refreshOpenTrades(state, candidates, universeTickerMap);
