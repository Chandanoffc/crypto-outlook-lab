...
async function recoverRuntimeStateFromTradeEvents(stateInput = {}) {
  ...
  for (const row of rows) {
    ...
    const current = tradeMap.get(row.trade_id) || {
      ...
      entryZoneLow: Number(metadata.entryZoneLow) || Number(row.entry_price) || 0,
      entryZoneHigh: Number(metadata.entryZoneHigh) || Number(row.entry_price) || 0,
      stopLoss: Number(row.stop_loss) || 0,
      tp1: Number(row.tp1) || 0,
      tp2: Number(row.tp2) || 0,
      currentTarget: Number(row.tp2 ?? row.tp1) || 0,
      tp1Hit: false,
      quantity: Number(row.quantity) || 0,
      leverage: Number(row.leverage) || TRADEZ_AUTO_LEVERAGE,
      ...
      keyDetails: "",
      signalNote: "",
      pricePrecision: Number(metadata.pricePrecision) || 2,
      ...
      lastPrice: Number(row.entry_price) || 0,
    ...
    current.entryPrice = Number(row.entry_price) || current.entryPrice;
    current.entryZoneLow = Number(metadata.entryZoneLow) || current.entryZoneLow;
    current.entryZoneHigh = Number(metadata.entryZoneHigh) || current.entryZoneHigh;
    current.stopLoss = Number(row.stop_loss) || current.stopLoss;
    current.tp1 = Number(row.tp1) || current.tp1;
    current.tp2 = Number(row.tp2) || current.tp2;
    current.currentTarget = current.tp2 || current.tp1 || current.currentTarget;
    current.quantity = Number(row.quantity) || current.quantity;
    current.leverage = Number(row.leverage) || current.leverage;
    current.marginUsed = Number(row.margin_used) || current.marginUsed;
    current.qualityScore = Number(row.quality_score) || current.qualityScore;
    current.pricePrecision = Number(metadata.pricePrecision) || current.pricePrecision;
    current.detectedAt = row.detected_at ? new Date(row.detected_at).getTime() : current.detectedAt;
    current.openedAt = row.opened_at ? new Date(row.opened_at).getTime() : current.openedAt;
    current.lastEventType = String(row.event_type || current.lastEventType || "");
    current.closedAt = row.closed_at ? new Date(row.closed_at).getTime() : current.closedAt;
    current.exitPrice = Number(row.exit_price) || current.exitPrice;
    current.returnPct = Number.isFinite(Number(row.return_pct)) ? Number(row.return_pct) : current.returnPct;
    current.pnlUsd = Number.isFinite(Number(row.pnl_usd)) ? Number(row.pnl_usd) : current.pnlUsd;
    current.balanceAfter = Number.isFinite(Number(row.balance_after)) ? Number(row.balance_after) : current.balanceAfter;
    if (String(row.event_type || "").toLowerCase() === "tp1_hit") {
      current.tp1Hit = true;
  ...
  for (const trade of tradeMap.values()) {
    ...
    const isClosed = lastEventType === "tp_hit" || lastEventType === "sl_hit" || lastEventType === "break_even_exit";
    latestScanAt = Math.max(latestScanAt, Number(trade.closedAt || trade.openedAt || 0));
    if (Number.isFinite(trade.balanceAfter)) {
