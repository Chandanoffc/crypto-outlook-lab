...
const { insertSignalEvent, insertTradeEvent, listTradeEventsByStrategy, queryCompetitionStats } = require("./neon-db");
...
const TRADEZ_AUTO_MIN_EXECUTION_THRESHOLD = 48;
...
async function openTradezPaperTrade(state, candidate, baseUrl) {
...
async function openTradezQualifiedTrades(state, candidates, baseUrl) {
  ...
  for (const candidate of eligible) {
    ...
    if (await openTradezPaperTrade(state, candidate, baseUrl)) opened.push(candidate);
  }
  return opened;
}
...
async function runTradezScan(stateInput = {}, options = {}) {
  ...
  try {
    ...
    const openedTrades = state.autoEnabled ? await openTradezQualifiedTrades(state, state.lastCandidates, baseUrl) : [];
