...
function sanitizePaperTrade(t = {}) {
  return {
    ...
    leverage:      sanitizeNumber(t.leverage, 1),  // stored per-position for backward compat
  };
}
...
function detectSignal(data, pricePrecision = 2) {
  ...
  // 4. Price position relative to EMA20 — tightened to 1.0 ATR (was 1.5)
  const distFromEma20 = Math.abs(currentPrice - ema20Last);
  const atEma20 = distFromEma20 <= atr * 0.8;
  const closeToEma20 = distFromEma20 <= atr * 1.0;
  ...
  // ATR must be at least 2.5% of price. At 3×ATR (TP1) that gives ~7.5% min,
  // at 5×ATR (TP2) that gives ~12.5% min rising to 25% on 5% ATR coins.
  const atrPct = entry > 0 ? atr / entry : 0;
  if (atrPct < 0.025) return null; // coin too stable for meaningful targets — skip
