...
function defaultState() {
  return { alerts: {}, lastScan: 0 };
}

async function runZenStrategy_Scan(state, { webhook } = {}) {
  const now = Date.now();
  ...
  for (const symbol of symbols) {
    try {
      // Cooldown check — skip if alerted in the last 6h
      const lastAlert = state.alerts[symbol] || 0;
      if (now - lastAlert < ALERT_COOLDOWN_MS) continue;

      // Fetch klines for pivot calculation (need prior period candle)
      ...
      if (dailyK.length < 2 || weeklyK.length < 2 || monthlyK.length < 2) continue;

      // Use the PRIOR completed period for pivot (not current forming candle)
      const daily   = calcPivot(dailyK[dailyK.length - 2]);
      const weekly  = calcPivot(weeklyK[weeklyK.length - 2]);
      const monthly = calcPivot(monthlyK[monthlyK.length - 2]);

      const price = dailyK[dailyK.length - 1].close;

      // Only scan longs (strategy is bullish bounce/breakout)
      const hit =
      ...
      if (hit && hit.tp1 > price && hit.sl < price) {
        ...
        if (rr < 1.0) continue; // skip poor R:R

        const signal = { symbol, entry: price, ...hit };
        await sendZenAlert(webhook, signal);
        state.alerts[symbol] = now;
        signals++;
      }
    } catch (e) {
  ...
  state.lastScan = now;
  return { signals, symbolsScanned: symbols.length, errors: errors.slice(0, 5) };
}
