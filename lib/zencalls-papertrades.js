...
// `stratState` is optional — when passed, closed trades mark the matching
// strategy signal as "closed" so the scanner can re-fire on the next setup.

async function checkPaperTrades(state, { webhook, stratState } = {}) {
  const now    = Date.now();
  ...
  for (const trade of open) {
    ...
    if (newStatus) {
      ...
      // Only mark the strategy signal as closed on real TP/SL outcomes — NOT expiry.
      // Expiry means the trade timed out without a resolution; the setup may still be
      // valid. Closing the signal on expiry would cause a mass re-blast the next time
      // many trades expire simultaneously (e.g. all opened at the same time after a reset).
      if (stratState?.signals && newStatus !== "expired") {
        const sig = stratState.signals.find(s => s.symbol === trade.symbol && s.status === "active");
