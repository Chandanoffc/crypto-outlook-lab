...
async function fetchRecentGraduates() {
  try {
    // Use both sort fields to maximize fresh graduate coverage
    const url = "https://frontend-api-v3.pump.fun/coins?complete=true&sort=created_timestamp&order=DESC&limit=100&includeNsfw=false";
    ...
    return coins.filter(c => {
      if (!c.mint || !c.created_timestamp) return false;
      // Use created_timestamp as graduation proxy (when pump.fun marks complete=true)
      const age = now - c.created_timestamp;
      return age <= MAX_GRAD_AGE_MS;
    });
  } catch { return []; }
}
...
async function runMemeAutoScan(state, { webhook } = {}) {
  ...
  for (let i = 0; i < candidates.length; i += BATCH) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      const ageMs        = grad?.created_timestamp ? now - grad.created_timestamp : null;
