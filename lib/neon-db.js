...
/**
 * Atomically claim the scan lock for an engine.
 * Stamps lastScanAt only when the stored value is older than (now - cooldownMs).
 * Returns true if this caller owns the lock, false if another scan already ran.
 */
async function tryClaimScanLock(engine, now, cooldownMs = 60_000) {
  ...
  try {
    ...
    const result = await client.query(
      `
        ...
        VALUES ($2, jsonb_build_object('lastScanAt', $1::bigint), NOW())
        ON CONFLICT (engine) DO UPDATE
          SET state      = jsonb_set(runtime_states.state, '{lastScanAt}', to_jsonb($1::bigint), true),
              updated_at = NOW()
          WHERE COALESCE((runtime_states.state ->> 'lastScanAt')::bigint, 0) < $3
      `,
      [now, asText(engine, 80) || "default", cutoff]
    );
...
/**
 * Atomically claim the mark lock for an engine.
 * Does a conditional UPDATE that only succeeds when the stored lastMarkAt
 * is older than (now - cooldownMs). Returns true if this caller owns the
 * lock (proceed with marking), false if another concurrent request beat us.
 * Using a single SQL statement makes it race-free — Postgres serialises it.
 */
async function tryClaimMarkLock(engine, now, cooldownMs = 30_000) {
  if (!hasDatabase()) return true; // dev/no-DB: always proceed
  await ensureSchema();
  const client = await getPool().connect();
  try {
    const cutoff = now - cooldownMs;
    const result = await client.query(
      `
        ...
        SET state      = jsonb_set(state, '{paper,lastMarkAt}', to_jsonb($1::bigint), true),
            updated_at = NOW()
        WHERE engine = $2
          AND COALESCE((state -> 'paper' ->> 'lastMarkAt')::bigint, 0) < $3
      `,
      [now, asText(engine, 80) || "default", cutoff]
    );
    return result.rowCount > 0; // true = we got the lock; false = someone else did
  } finally {
    client.release();
  }
}
...
async function listTradeEventsByStrategy(strategy, limit = 500) {
  if (!hasDatabase()) return buildSkipped("DATABASE_URL not configured.");
  await ensureSchema();
  const client = await getPool().connect();
  try {
    ...
    const result = await client.query(
      `
        ...
        LIMIT $2
      `,
      [asText(strategy, 80) || "unknown", safeLimit]
    );
    return {
      ok: true,
      rows: result.rows || [],
...
async function queryCompetitionStats(lookbackHours = 720) {
  if (!hasDatabase()) return buildSkipped("DATABASE_URL not configured.");
  await ensureSchema();
  const client = await getPool().connect();
  try {
    const safeLookback = Math.max(1, Math.round(Number(lookbackHours) || 720));
    const result = await client.query(
      `
        SELECT
          strategy,
          event_type,
          COUNT(*) AS count,
          COALESCE(AVG(CASE WHEN return_pct IS NOT NULL AND return_pct <> 0 THEN return_pct END), 0) AS avg_return_pct,
          COALESCE(SUM(COALESCE(pnl_usd, 0)), 0) AS total_pnl_usd,
          COALESCE(AVG(quality_score), 0) AS avg_quality
        FROM trade_events
        WHERE strategy IN ('house_trend', 'ema_book')
          AND event_type IN ('tp_hit', 'sl_hit', 'break_even_exit')
          AND event_time >= NOW() - ($1 * INTERVAL '1 hour')
        GROUP BY strategy, event_type
        ORDER BY strategy, event_type
      `,
      [safeLookback]
    );
    return { ok: true, rows: result.rows || [], lookbackHours: safeLookback };
  } finally {
    client.release();
  }
}
...
module.exports = {
  ...
  tryClaimScanLock,
  tryClaimMarkLock,
  listTradeEventsByStrategy,
  queryCompetitionStats,
};
