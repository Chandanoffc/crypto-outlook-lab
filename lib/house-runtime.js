...
const { hasDatabase, getRuntimeState, listTradeEventsByStrategy } = require("./neon-db");
...
const DEFAULT_QUALITY_THRESHOLD = 48;
const BINANCE_SPOT_BASE_URL = "https://data-api.binance.vision";
const PRIORITY_SCAN_COUNT = 18;
...
const HOUSE_DRAWDOWN_HALF_FLOOR = 0.80;   // below 80% of strategy start → half sizing
const HOUSE_DRAWDOWN_BLOCK_FLOOR = 0.65;  // below 65% of strategy start → block new trades
...
const MIN_RR = 1.08;
const MIN_PROJECTED_MOVE_PCT = 0.5;
const STRATEGY_VERSION = 6;
const HOUSE_MIN_ENTRY_SCORE = -20;
const HOUSE_MIN_REFINED_SCORE = 38;
const MIN_NEAREST_TARGET_RR = 0.9;
...
function isClosingTradeEvent(eventType) {
  ...
  return normalized === "tp" || normalized === "sl";
}
...
function buildResetRuntimeState(currentState = {}) {
  ...
  return sanitizeRuntimeState({
    ...
    freshStart: true,
    resetAt: Date.now(),
    lastStatusMessage: "Background House engine reset. Starting from a fresh book.",
    lastStatusTone: "neutral",
  });
}
...
function applyCandidateConfirmation(candidate, ticker, confirmation, threshold) {
  ...
  if (candidate.compressedStructure) entryQualityScore -= 6;
  if (candidate.extremeCompressedStructure) entryQualityScore -= 2;
  if (candidate.lowVolatilityChop) entryQualityScore -= 6;
  if (candidate.extremeLowVolatilityChop) entryQualityScore -= 8;
  ...
  return candidates
    .filter(
      (candidate) =>
        ...
        (hasGoodTradingVolume(candidate.quoteVolume) || candidate.quoteVolume >= HIGH_VOLUME_FLOOR * 0.45) &&
        candidate.rr >= MIN_RR &&
...
function buildHouseEntryAlertEvent(trade) {
  const pair = formatBannerPair(trade.symbol);
  const title = `Auto Trade ${trade.side} ${trade.symbol}`;
  const event = {
    time: trade.openedAt,
    title,
    symbol: trade.symbol,
    ...
    strategy: trade.strategyLabel || "House Trend",
  ...
  event.message = [
    ...
    `Take Profit: ${formatPrice(trade.takeProfit, trade.pricePrecision || 2)}`,
    `RR: ${Number.isFinite(Number(trade.rr)) ? Number(trade.rr).toFixed(2) + "R" : "—"}`,
    "",
    `Why: ${trade.entryReason || "House Trend setup qualified."}`,
  ].join("\n");
  event.formattedMessage = event.message;
  return event;
}

function buildHouseExitAlertEvent(reason, trade, closedTrade) {
  const pair = formatBannerPair(trade.symbol);
  const event = {
    time: closedTrade.closedAt,
    title:
      reason === "TP"
        ? `Auto Trade TP ${trade.symbol}`
        : reason === "SL"
          ? `Auto Trade SL ${trade.symbol}`
          : `Auto Trade BE ${trade.symbol}`,
    symbol: trade.symbol,
    pair,
    type: "house",
    direction: trade.side,
    strategy: trade.strategyLabel || "House Trend",
    deliveryType: reason === "TP" ? "tp_hit" : reason === "SL" ? "sl_hit" : "break_even_exit",
    qualityScore: trade.qualityScore,
    entryPrice: trade.entryPrice,
    stopLoss: trade.stopLoss,
    tp1: trade.takeProfit,
  ...
  event.message = [
    reason === "TP" ? "SUCCESS.. TP Hit✅" : reason === "SL" ? "Oh No, SL HIT❌" : "Protected at Entry🛡️",
    `${pair} • ${trade.side} • ${trade.leverage}x`,
...
async function maybeSendHouseRuntimeAlert(state, event, meta, baseUrl) {
  ...
  try {
    ...
    updateAlertDeliveryResult(state, "up", "Background House Discord delivery succeeded.");
  } catch (error) {
    updateAlertDeliveryResult(state, "down", error.message || "Background House Discord delivery failed.");
    logActivity(state, `House Discord delivery failed • ${error.message || "Unknown error"}`, "down");
  }
}
...
function drawdownMultiplier(balance, startingBalance) {
  ...
  if (ratio < HOUSE_DRAWDOWN_BLOCK_FLOOR) return 0.0;
  if (ratio < HOUSE_DRAWDOWN_HALF_FLOOR) return 0.5;
  return 1.0;
}

async function openTradeFromCandidate(state, candidate, baseUrl, tradezOpenSymbols = new Set()) {
  ...
  // Cross-engine symbol lock — block trades on symbols already open in tradez-runtime.
  if (tradezOpenSymbols.has(candidate.symbol)) return false;

  const strategyBalanceBefore = Number(state.strategyBalances?.[strategyId]) || STRATEGY_START_BALANCE;

  // Drawdown circuit breaker.
  const ddMult = drawdownMultiplier(strategyBalanceBefore, STRATEGY_START_BALANCE);
  ...
  const qualityMult = qualityMarginMultiplier(candidate.refinedQualityScore ?? candidate.qualityScore);
  const marginBudget = Math.min(
    freeCapital,
    Math.max(strategyBalanceBefore * 0.16, freeCapital / slotsRemaining) * qualityMult * ddMult
  );
  ...
  if (state.alertDelivery?.notifyEntries !== false) {
    await maybeSendHouseRuntimeAlert(
      state,
      buildHouseEntryAlertEvent(openedTrade),
      {
        source: "house_cron",
        strategy: "house_trend",
...
async function closeTrade(state, tradeId, reason, exitPrice, precisionHint, baseUrl) {
  ...
  await safeInsertTradeEvent({
    ...
    strategy: "house_trend",
    strategyVersion: STRATEGY_VERSION,
    eventType: reason === "TP" ? "tp" : reason === "BE" ? "break_even_exit" : "sl",
    symbol: trade.symbol,
    side: trade.side,
  ...
  if (state.alertDelivery?.notifyExits !== false) {
    await maybeSendHouseRuntimeAlert(
      state,
      buildHouseExitAlertEvent(reason, trade, closedTrade),
      {
        source: "house_cron",
        strategy: "house_trend",
...
async function pruneUnavailableOpenTrades(state, universe, baseUrl) {
  ...
  for (const trade of [...state.openTrades]) {
    ...
    await closeTrade(state, trade.id, "BE", exitPrice, trade.pricePrecision, baseUrl);
  }
}
...
async function refreshOpenTrades(state, candidates, tickerLookup = universeTickerMap, baseUrl) {
  if (!state.openTrades.length) return;
  ...
  for (const trade of trades) {
    ...
    if (candidate) tightenTradeProtection(state, trade, candidate);
    const hitTarget =
      trade.side === "Long"
    ...
    if (hitTarget) {
      await closeTrade(state, trade.id, "TP", trade.takeProfit, candidate?.pricePrecision ?? trade.pricePrecision, baseUrl);
    } else if (hitStop) {
      const stopReason = trade.breakEvenArmed || trade.profitLockArmed ? "BE" : "SL";
      await closeTrade(state, trade.id, stopReason, trade.stopLoss, candidate?.pricePrecision ?? trade.pricePrecision, baseUrl);
    }
  }
}

async function openQualifiedTrades(state, candidates, baseUrl, tradezOpenSymbols = new Set()) {
  const opened = [];
  ...
  for (const candidate of eligible) {
    ...
    if (await openTradeFromCandidate(state, candidate, baseUrl, tradezOpenSymbols)) {
      opened.push({
        strategyId: spec.id,
        strategyLabel: spec.label,
        symbol: candidate.symbol,
...
async function fetchUniverseSpotTickersFallback(activeSymbols) {
  const tickers = await fetchJson(`${BINANCE_SPOT_BASE_URL}/api/v3/ticker/24hr`, "Spot 24H tickers");
  return mapUniverseTickers(tickers, activeSymbols);
}
...
async function fetchSpotCoreSnapshotDirect(resolved, interval) {
  ...
  return fetchFirstSuccessful(candidates, async (symbol) => {
    const requests = await Promise.allSettled([
      fetchJson(`${BINANCE_SPOT_BASE_URL}/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=240`, "Spot klines"),
      fetchJson(`${BINANCE_SPOT_BASE_URL}/api/v3/ticker/24hr?symbol=${symbol}`, "Spot 24H ticker"),
      fetchJson(`${BINANCE_SPOT_BASE_URL}/api/v3/depth?symbol=${symbol}&limit=100`, "Spot depth"),
      fetchJson(`${BINANCE_SPOT_BASE_URL}/api/v3/aggTrades?symbol=${symbol}&limit=400`, "Spot agg trades"),
    ]);
...
async function fetchSnapshotViaMarketApi(token, interval, baseUrl) {
  return fetchJson(
    `${resolveBaseUrl(baseUrl)}/api/market?symbol=${encodeURIComponent(token)}&interval=${encodeURIComponent(interval)}`,
    "Runtime market snapshot"
  );
}

async function fetchDirectSnapshot(token, interval, baseUrl) {
  try {
    return await fetchSnapshotViaMarketApi(token, interval, baseUrl);
...
async function fetchHigherTimeframeEntries(symbol, config) {
  try {
    ...
    return fetchJson(
      `${BINANCE_SPOT_BASE_URL}/api/v3/klines?symbol=${symbolInfo.baseAsset}${QUOTE_ASSET}&interval=${config.interval}&limit=180`,
...
async function runHouseScan(stateInput = {}, options = {}) {
  ...
  if (
    ...
    updateAlertDeliveryResult(state, "neutral", "Background House Discord delivery is armed.");
  }
  ...
  try {
    ...
    const results = await mapWithConcurrency(selection.batch, ANALYSIS_CONCURRENCY, async (symbolInfo) => {
      try {
        return analyzeSnapshot(await fetchDirectSnapshot(symbolInfo.symbol, state.interval, baseUrl));
      } catch (error) {
        snapshotFailures += 1;
        return null;
      }
    ...
    const confirmationSymbols = Array.from(
      new Set([
        ...rawCandidates.slice(0, 32).map((candidate) => candidate.symbol),
    ...
    let confirmationFallbacks = 0;
    confirmationResults.forEach((entry) => {
      if (entry?.summary?.["1h"]?.label === "Unavailable" || entry?.summary?.["4h"]?.label === "Unavailable") {
        confirmationFallbacks += 1;
      }
    });

    await refreshOpenTrades(state, candidates, universeTickerMap, baseUrl);
    const qualified = highQualityCandidates(candidates, state.qualityThreshold);

    // Cross-engine lock: read tradez-runtime open positions once per scan (non-blocking).
    let tradezOpenSymbols = new Set();
    if (hasDatabase()) {
      try {
        const tradezRaw = await getRuntimeState("tradez_auto_trade");
        tradezOpenSymbols = new Set((tradezRaw?.state?.openTrades || []).map((t) => t.symbol));
      } catch (_) {
        // Non-blocking — if DB read fails, proceed without the lock.
      }
    }

    const opened = await openQualifiedTrades(state, candidates, baseUrl, tradezOpenSymbols);

    const degradedSummary =
      snapshotFailures || confirmationFallbacks
        ? ` Degraded scan: ${snapshotFailures}/${selection.batch.length} symbol snapshots failed, ${confirmationFallbacks}/${confirmationSymbols.length || 1} HTF confirmations fell back.`
        : "";
    ...
    if (opened.length) {
      ...
      setRuntimeStatus(
        state,
        `Opened ${opened.length} new house-strategy trade${opened.length > 1 ? "s" : ""}. ${state.openTrades.length}/${MAX_CONCURRENT_TRADES} positions active, led by ${lead.symbol}.${degradedSummary}`,
        lead.trade?.tone || lead.bias?.tone || "up"
      );
    } else if (state.openTrades.length) {
      setRuntimeStatus(
        state,
        `Monitoring ${state.openTrades.length} active paper position${state.openTrades.length > 1 ? "s" : ""} while the background universe scan continues.${degradedSummary}`,
        snapshotFailures ? "neutral" : state.openTrades.some((trade) => trade.side === "Long") ? "up" : "down"
      );
    } else if (qualified.length) {
      setRuntimeStatus(
        state,
        `${qualified.length} setups qualify, but margin or cooldown rules are holding entries for now.${degradedSummary}`,
        snapshotFailures ? "neutral" : qualified[0].bias.tone
      );
    } else {
      if (manual) {
        logActivity(
      ...
      setRuntimeStatus(
        state,
        snapshotFailures || confirmationFallbacks
          ? `No trade opened. Scanner degraded while waiting for the effective house bar of Q${effectiveHouseQualityGate(state.qualityThreshold)}. ${snapshotFailures}/${selection.batch.length} symbol snapshots failed and ${confirmationFallbacks}/${confirmationSymbols.length || 1} HTF confirmations fell back.`
