...
async function recoverRuntimeStateFromTradeEvents(stateInput = {}) {
  ...
  for (const row of rows) {
    ...
    const current = tradeMap.get(row.trade_id) || {
      ...
      takeProfit: Number(row.tp2 ?? row.tp1) || 0,
      quantity: Number(row.quantity) || 0,
      leverage: Number(row.leverage) || DEFAULT_LEVERAGE,
      marginUsed: Number(row.margin_used) || 0,
      qualityScore: Number(row.quality_score) || 0,
      biasScore: 0,
      targetReturnPct: 0,
      stopReturnPct: 0,
      projectedMovePct: Number(metadata.projectedMovePct) || 0,
      rr: Number(metadata.rr) || 0,
      entryScore: Number(metadata.entryScore) || 0,
    ...
    current.takeProfit = Number(row.tp2 ?? row.tp1) || current.takeProfit;
    ...
    current.projectedMovePct = Number(metadata.projectedMovePct) || current.projectedMovePct;
    current.rr = Number(metadata.rr) || current.rr;
    current.entryScore = Number(metadata.entryScore) || current.entryScore;
  ...
  for (const trade of tradeMap.values()) {
    ...
    const isClosed = lastEventType === "tp" || lastEventType === "sl";
