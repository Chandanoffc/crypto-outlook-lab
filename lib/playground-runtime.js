...
async function sendPerpsAlerts(state, houseState, tradezState, baseUrl) {
  if (!state.perps.scannerEnabled || !isDiscordWebhook(state.perps.webhook)) return 0;
  const calls = mergePerpsCalls(houseState, tradezState)
  ...
  const candidates = mergePerpsCandidates(houseState, tradezState)
  ...
  const outbound = [
    ...calls.map((call) => ({
      id: `call:${call.id}`,
      title: `${call.engineLabel} opened ${call.symbol}`,
      payload: createPerpsAlertPayload(call),
    })),
  ...
  for (const item of outbound) {
    ...
    try {
      await sendNotify(baseUrl, PERPS_MODULE, state.perps.webhook, item.title, item.payload, {
        source: "playground_perps",
        strategy: "perps_alerts",
        eventType: "scanner_signal",
      });
      pushSentId(state, PERPS_MODULE, item.id);
  ...
  if (modules.includes(PERPS_MODULE)) {
    try {
      ...
      pushModuleLog(state, PERPS_MODULE, "scanLog", {
        tone: "neutral",
        message: manual ? "Manual perps background refresh completed." : "Perps background refresh completed.",
      });
      summary.perpsSent = await sendPerpsAlerts(state, houseState, tradezState, baseUrl);
    } catch (error) {
