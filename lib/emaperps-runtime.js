...
function closePaperPos(state, pos, closePrice, reason, now) {
  ...
  // pnlPct = leveraged % price move from entry to close (what traders actually care about)
  const priceDiffPct = (diff / pos.entryPrice) * lev * 100;
  const pnlPct = Math.round(priceDiffPct * 100) / 100;
...
function detectSignal(data, pricePrecision = 2, context = {}) {
  ...
  if (uptrend && srLevelsUsable) {
    // Long signal cases — TP1 = next swing resistance (structural target), SL below the level
  ...
  if (downtrend && side === null && srLevelsUsable) {
    // Short signal cases — TP1 = next swing support (structural target), SL above the level
  ...
  // ATR-based TP2 for A-F signals — swing R2 is often too close or unclear.
  // P/Q signals already set tp2 = price ± 5×ATR above.
