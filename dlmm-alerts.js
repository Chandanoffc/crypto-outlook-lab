const PLAYGROUND_STORAGE_KEY = "soloris-dlmm-alerts-v1";
...
function renderDlmmModule() {
  ...
  return `
    <div class="playground-module-stack">
      ...
      ${renderPerformanceSummary(metrics, DLMM_MODULE)}
...
function render() {
  ...
  if (!content) return;
  content.innerHTML = renderDlmmModule();
}
...
async function init() {
  ...
  await fetchPlaygroundRuntimeState();
  await syncPlaygroundRuntimeSettings(DLMM_MODULE);
  await refreshDlmmData({ source: "init" });
  scheduleDlmmScanner();
}
