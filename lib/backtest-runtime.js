 ...
 * relevant constants — from emaperps-runtime.js.
...
async function runBacktest(opts = {}) {
  const engineId = String(opts.engine || "emaperps").toLowerCase();
  const loader = ENGINES[engineId];
  if (!loader) throw new Error(`Unknown engine "${opts.engine}". Use "emaperps".`);
