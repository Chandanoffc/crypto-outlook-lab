...
const MIN_ALERT_QUALITY = 88;     // raised 83→88: Q83-87 bucket 0W/5L on 31 live trades; quality score is not predictive below 88
...
// Conviction-weighted sizing — a Q92 setup gets ~2x the allocation of a Q78 one.
...
// BTC 4H macro trend — used as hard alignment filter (don't trade alt longs into BTC downtrend)
// Returns "up" / "down" / "neutral". Neutral fires when EMA20 and EMA50 are within 1.5% of
// each other — BTC is ranging, so individual token trend should drive direction, not BTC macro.
let btcTrendCache = { trend: null, fetchedAt: 0 };
const BTC_TREND_TTL_MS = 2 * 60 * 60 * 1000;
const BTC_RANGING_BAND = 0.015; // EMA20/EMA50 spread < 1.5% → call it neutral

async function getBtcTrend() {
  const now = Date.now();
  if (btcTrendCache.trend && (now - btcTrendCache.fetchedAt) < BTC_TREND_TTL_MS) return btcTrendCache.trend;
  try {
    const klines = await fetchJson(`${BINANCE_FAPI}/klines?symbol=BTCUSDT&interval=${HTF_INTERVAL}&limit=60`, "BTC 4H trend");
    const closes = klines.map(k => parseFloat(k[4]));
    const e20 = ema(closes, 20), e50 = ema(closes, 50);
    const last = closes.length - 1;
    let trend = null;
    if (e20[last] != null && e50[last] != null) {
      const spread = Math.abs(e20[last] - e50[last]) / e50[last];
      if (spread < BTC_RANGING_BAND) trend = "neutral"; // EMAs too close — ranging market
      else trend = e20[last] > e50[last] ? "up" : "down";
    }
    btcTrendCache = { trend, fetchedAt: now };
    return trend;
  } catch (_) { return btcTrendCache.trend ?? null; }
}
...
// Wilder's ADX — measures trend strength (0-100). >25 = trending, <18 = ranging/choppy.
// Ranging markets produce frequent EMA crossovers that immediately reverse — ADX < 18
// reliably identifies these low-quality regimes and filtering them out improves WR.
function adxValue(candles, period = 14) {
  ...
  for (let i = 1; i < candles.length; i++) {
    ...
    trs.push(tr);
    plusDMs.push(upMove > 0 && upMove > downMove ? upMove : 0);
    minusDMs.push(downMove > 0 && downMove > upMove ? downMove : 0);
  }
  // Wilder's smoothing: first value = sum of first period, then smooth incrementally
...
// RSI divergence: price making new extremes that RSI doesn't confirm → exhaustion signal.
// Classic TA pattern with genuine predictive value — much stronger than just "RSI in zone".
function hasBullishDivergence(candles1h, rsi14, lookback = 10) {
  const n = candles1h.length;
  if (n < lookback * 2 + 5 || rsi14.length < lookback * 2) return false;
  const recentPriceLow = Math.min(...candles1h.slice(-lookback).map(c => c.low));
  const priorPriceLow  = Math.min(...candles1h.slice(-lookback * 2, -lookback).map(c => c.low));
  const recentRsiLow   = Math.min(...rsi14.slice(-lookback).filter(v => v != null));
  const priorRsiLow    = Math.min(...rsi14.slice(-lookback * 2, -lookback).filter(v => v != null));
  if (!isFinite(recentRsiLow) || !isFinite(priorRsiLow)) return false;
  return recentPriceLow < priorPriceLow * 0.998 && recentRsiLow > priorRsiLow + 2;
}

function hasBearishDivergence(candles1h, rsi14, lookback = 10) {
  const n = candles1h.length;
  if (n < lookback * 2 + 5 || rsi14.length < lookback * 2) return false;
  const recentPriceHigh = Math.max(...candles1h.slice(-lookback).map(c => c.high));
  const priorPriceHigh  = Math.max(...candles1h.slice(-lookback * 2, -lookback).map(c => c.high));
  const recentRsiHigh   = Math.max(...rsi14.slice(-lookback).filter(v => v != null));
  const priorRsiHigh    = Math.max(...rsi14.slice(-lookback * 2, -lookback).filter(v => v != null));
  if (!isFinite(recentRsiHigh) || !isFinite(priorRsiHigh)) return false;
  return recentPriceHigh > priorPriceHigh * 1.002 && recentRsiHigh < priorRsiHigh - 2;
}
...
function sanitizePaperTrade(t = {}) {
  return {
    ...
    signalLabel:   String(t.signalLabel || ""),
...
function defaultRuntimeState() {
  return {
    signals: [],
    lastScanAt: 0,
    lastCronAt: 0,   // timestamp of the last successful upbit-cron ping — lets the
                     // dashboard warn when the background cron has gone silent
    scanCursor: 0,
...
function closePaperPos(state, pos, closePrice, reason, now) {
  ...
  // Returns true if it actually closed, false if it was a duplicate (callers must
  // check the return value before firing Discord alerts).
  ...
  // After a losing SL hit, cool down that symbol for 6 hours to prevent
  // the engine from immediately re-entering the same unfavourable setup.
...
async function markPaperPositions(state, now, baseUrl = null) {
  ...
  for (const pos of state.paper.openPositions) {
    ...
    if (!price) {
      // Price fetch failed — apply a 24h failsafe: if we haven't had a successful
      // mark in 24 hours, force-close at the last known price rather than letting
      // the position sit in limbo (delisted symbol, persistent API failure, etc.).
...
// the engine from repeatedly hammering a symbol in an unfavourable regime
// (e.g. INJUSDT choppy range that the confluence model consistently mis-reads).
...
function detectSignal(data, pricePrecision = 2, context = {}) {
  ...
  // 0a. Hard block: minimum 24H volume — low-liquidity coins amplify small BTC dips 6-8×,
  // producing violent SL hits that aren't recoverable by any technical filter downstream.
  // Raised to $200M: focuses signals on top-tier perps with institutional-grade liquidity.
  if (quoteVolume > 0 && quoteVolume < 200_000_000) return null;

  // 0b. Hard block: 4H trend must not oppose 1H trend — trading counter-trend kills win rate
  if (trend4h !== null && trend4h !== ema1hTrend) return null;

  // 0b. BTC macro alignment — only trade alts in the direction the BTC 4H trend supports.
  // Backtest: Long WR 28% vs Short WR 50% over 90d when BTC was in uptrend only 42% of
  // the time. Longs taken into BTC downtrend are the primary source of -1% expectancy drag.
  // Soft fallback: if BTC trend unavailable (API failure), skip the block — don't suppress signals.
  // "neutral" = BTC EMAs within 1.5% — ranging market, allow both directions.
  // Only block when BTC is clearly trending against the trade.
  if (btcTrend !== null && btcTrend !== "neutral") {
    if (isLong  && btcTrend === "down") return null;
    if (!isLong && btcTrend === "up")   return null;
  }

  // 0c. ADX trend-strength filter — skip signals in ranging markets (ADX < 18).
  // EMA crossovers in choppy regimes reverse immediately; this removes the cleanest
  // category of false entries without touching the quality scoring system.
  const adx4h = candles4h.length >= 30 ? adxValue(candles4h, 14) : 0;
  if (adx4h > 0 && adx4h < 18) return null;

  // 0d. Funding Rate Exhaustion — bypass all trend filters when the market is
  // structurally crowded. When funding > +0.05%/8h, longs are paying heavily and the
  // crowd is overextended → fade with a Short. When funding < -0.05%, shorts crowded → Long.
  // This fires in ranging and trending markets alike and is regime-agnostic.
  const FUNDING_EXTREME = 0.05;
  if (Math.abs(fundingRate) >= FUNDING_EXTREME && rsiLast != null) {
    const fadeLong  = fundingRate < -FUNDING_EXTREME; // shorts crowded → buy
    const fadeShort = fundingRate >  FUNDING_EXTREME; // longs crowded → sell
    const fadeSide  = fadeLong ? "Long" : "Short";
    // Require one candle closing in the fade direction (momentum starting)
    const candleConfirms = fadeLong ? (candle.close > candle.open) : (candle.close < candle.open);
    // RSI must not already be extended in the wrong direction
    const rsiOkFunding = fadeLong ? rsiLast < 65 : rsiLast > 35;
    if (candleConfirms && rsiOkFunding) {
      const atrPctF = currentPrice > 0 ? atr / currentPrice : 0;
      if (atrPctF >= 0.020 && atrPctF <= 0.060) {
        const fundSL  = fadeLong
          ? Math.max(candle.low  - atr * 0.15, currentPrice - atr * 0.75)
          : Math.min(candle.high + atr * 0.15, currentPrice + atr * 0.75);
        const fundTP1 = fadeLong ? currentPrice + atr * 2.0 : currentPrice - atr * 2.0;
        const fundTP2 = fadeLong ? currentPrice + atr * 3.5 : currentPrice - atr * 3.5;
        const fundQ   = Math.min(68 + (Math.abs(fundingRate) > 0.08 ? 8 : 0) + (Math.abs(fundingRate) > 0.12 ? 7 : 0), 90);
        return {
          id: `${symbol}-${INTERVAL}-${fadeSide}-FUNDING-${candle.time}`,
          symbol, side: fadeSide, quality: fundQ, interval: INTERVAL,
          detectedAt: Date.now(), alertedAt: 0,
          entryPrice: currentPrice,
          sl:  Number(fundSL.toFixed(pricePrecision)),
          tp1: Number(fundTP1.toFixed(pricePrecision)),
          tp2: Number(fundTP2.toFixed(pricePrecision)),
          rr1: 2.0, rr2: 3.5,
          ...
          currentPrice, fundingRate, volume24h: quoteVolume,
          status: "active",
          factors: { fundingExhaustion: true },
          signalType: "FUNDING",
          reasonLabels: [
            `Funding ${fundingRate > 0 ? "+" : ""}${fundingRate.toFixed(3)}% — ${fundingRate > 0 ? "longs" : "shorts"} crowded`,
            "Sentiment exhaustion fade",
            `RSI ${Math.round(rsiLast)}`,
            `$${(quoteVolume / 1e6).toFixed(0)}M 24H vol`,
          ],
          pricePrecision, candleTime: candle.time,
        };
      }
    }
  }
  ...
  // Hard gate: when price is sitting AT EMA20 (the tightest, highest-quality zone),
  // require a rejection wick confirming the EMA is acting as real support/resistance.
  // Without this, "at EMA20" entries are often pass-throughs that keep running
  // through the level, hitting SL for large losses (recent -16% to -37% SL trades
  // were all atEma20 entries with wickRejection: false).
  if (atEma20 && !wickRejection) return null;
  ...
  // Hard gate: MACD must be crossed or at least confirming.
  // macdApproaching alone (histogram compressing but no signal cross) predicts
  // momentum that never materialises — Q83-87 bucket was 0W/5L on those setups.
  if (!macdCrossed && !macdConfirming) return null;

  // RSI divergence: price makes new extreme but RSI doesn't confirm → momentum exhaustion.
  // Much more informative than a simple RSI zone check — detects hidden strength/weakness.
  const rsiDiv = isLong
    ? hasBullishDivergence(candles1h, rsi14)
    : hasBearishDivergence(candles1h, rsi14);
  factors.rsiDivergence = rsiDiv;
  factors.adxStrong     = adx4h >= 25;
  ...
  quality += rsiDiv ? 10 : 0;        // RSI divergence = strong exhaustion signal
  quality += adx4h >= 25 ? 5 : 0;   // ADX confirms an established trend, not choppy range
  ...
  // Ceiling: the SL formula scales with ATR (~1xATR), so on hyper-volatile coins
  // the SL distance as % of price blows up too — BTWUSDT had atrPct ~9% and the
  // SL hit for -45% pnlPct at 5x leverage. Cap at 6% (~30% max SL loss at 5x).
  if (atrPct > 0.060) return null;
  // SL: wick as structural reference + 0.15 ATR buffer, capped at 0.75×ATR from entry
  // Tighter stop improves R:R — the old 1×ATR stop was causing −20% to −30% pnlPct losses
  // Hard cap SL at 2% from entry regardless of ATR size.
  // Avg SL loss was -14.74% on 31 trades; ATR-only SL lets hyper-volatile alts
  // eat 10-15% PnL per loss. The 2% hard cap limits worst case to ~10% at 5× lev.
  const MAX_SL_PCT = 0.020;
  const slRaw = isLong
    ? Math.max(candle.low - atr * 0.15, entry - atr * 0.75)
    : Math.min(candle.high + atr * 0.15, entry + atr * 0.75);
  const sl = isLong
    ? Math.max(slRaw, entry * (1 - MAX_SL_PCT))
    : Math.min(slRaw, entry * (1 + MAX_SL_PCT));
  const riskAtr = atr;
  ...
  // Build reason labels — full trading rationale
  const reasonLabels = [];
  ...
  if (btcTrend === "up"   && isLong)   reasonLabels.push("BTC macro bullish");
  if (btcTrend === "down" && !isLong)  reasonLabels.push("BTC macro bearish");
  if (btcTrend === "neutral")          reasonLabels.push("BTC ranging — token-level trend drives direction");
  ...
  if (rsiDiv) reasonLabels.push(isLong ? "Bullish RSI divergence" : "Bearish RSI divergence");
  if (adx4h >= 25) reasonLabels.push(`ADX ${Math.round(adx4h)} trending`);
  if (adx4h >= 18 && adx4h < 25) reasonLabels.push(`ADX ${Math.round(adx4h)} building`);
...
async function runClaudePerps_Scan(stateInput = {}, options = {}) {
  ...
  // Mark open paper positions — null baseUrl suppresses close alerts here.
  // Close alerts are sent exclusively from the action=mark endpoint (lock-protected).
  await markPaperPositions(state, now, null);
  ...
  try {
    ...
    // Pre-fetch BTC 4H trend once for the whole scan batch — used as macro alignment filter
    ...
    for (const signal of results.filter(Boolean)) {
      ...
      // Inherit alertedAt from the evicted signal so a new candle (new signal ID) doesn't
      // re-fire the Discord alert for the same setup within the 4h cooldown window.
      const evicted = state.signals.find(s => s.symbol === signal.symbol && s.side === signal.side);
      const inheritedAlertedAt = (evicted && evicted.alertedAt > 0 && now - evicted.alertedAt < ALERT_COOLDOWN_MS)
        ? evicted.alertedAt : 0;
      state.signals = state.signals.filter(s => !(s.symbol === signal.symbol && s.side === signal.side));
      signal.alertedAt = inheritedAlertedAt;
      state.signals.unshift(signal);
      newSignals++;
      openPaperTrade(state, signal, now);
      if (!inheritedAlertedAt) await maybeSendAlert(state, signal, baseUrl);
    }
